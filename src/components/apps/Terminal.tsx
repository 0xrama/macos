...
// HowDare is defined as a separate component to avoid re-creation on each render
const HowDare = React.memo(function HowDare({
  setRMRF
}: {
...
export default class Terminal extends React.Component<object, TerminalState> {
  ...
  constructor(props: object) {
    ...
    this.commands = {
      ...
      clear: this.clear,
      help: this.help,
      xero: this.xero, // Add the hidden command here
      neofetch: this.neofetch // Add the neofetch command here
    };
  }

  // Define the hidden command
  xero = () => {
    this.generateResultRow(
      this.curInputTimes,
      <span>This is a hidden command response!</span>
    );
  };

  // Define the neofetch command
  neofetch = () => {
    const info = (
      <pre>{`
                    'c.          sriram@macbook.local
                 ,xNMM.          --------------------------------
               .OMMMMo           OS: macOS Tahoe 26.0 arm64
               OMMM0,            Host: Mac16,1
     .;loddo:' loolloddol;.      Kernel: 26.0.0
   cKMMMMMMMMMMNWMMMMMMMMMM0:    Uptime: 2 days, 4 hours, 11 mins
 .KMMMMMMMMMMMMMMMMMMMMMMMWd.    Packages: 21 (brew)
 XMMMMMMMMMMMMMMMMMMMMMMMX.      Shell: zsh 5.9
;MMMMMMMMMMMMMMMMMMMMMMMM:       Resolution: 1920x1243
 ...
 .XMMMMMMMMMMMMMMMMMMMMMMMMMMk   Terminal: ghostty
  .XMMMMMMMMMMMMMMMMMMMMMMMMK.   CPU: Apple M4 Pro
    kMMMMMMMMMMMMMMMMMMMMMMd     GPU: Apple M4 Pro
     ;KMMMMMMMWXXWMMMMMMMk.      Memory: 1630MiB / 24576MiB
       .cooc,.    .,coo:.`}</pre>
    );
    this.generateResultRow(this.curInputTimes, info);
  };
      ...
      <div key={`terminal-input-row-${id}`} flex>
        <div className="w-max hstack space-x-1.5">
          <span text-yellow-200>
            sriram@macbook-pro <span text-green-300>{this.getCurDirName()}</span>
          </span>
