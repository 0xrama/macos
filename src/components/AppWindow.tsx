...
// Static icon components are memoized to prevent unnecessary re-renders
const FullIcon = React.memo(function FullIcon({ size }: { size: number }) {
  return (
    <svg
      className="icon"
      viewBox="0 0 13 13"
...
const ExitFullIcon = React.memo(function ExitFullIcon({ size }: { size: number }) {
  return (
    <svg
      className="icon"
      viewBox="0 0 19 19"
...
const TrafficLights = React.memo(function TrafficLights({
  id,
  close,
  aspectRatio,
  max,
  ...
  return (
    <div className="traffic-lights flex flex-row absolute left-0 space-x-2 pl-2 mt-2">
      <button
...
const Window = React.memo(function Window(props: WindowProps) {
  ...
  const round = props.max ? "rounded-none" : "rounded-glass";
  const minimized = props.min
    ? "opacity-0 invisible transition-opacity duration-300"
    : "";
  const border = props.max ? "" : "border-[0.5px] border-black/6 dark:border-white/6";
  ...
  return (
    <Rnd
      ...
      className={`overflow-hidden ${round} ${border} window-shadow ${minimized} ${
        props.active ? "" : "rnd-inactive"
      }`}
      id={`window-${props.id}`}
    >
      <div
        className="window-bar relative h-7 text-center"
      ...
      >
        <TrafficLights
          ...
          close={props.close}
        />
        <span className="window-title font-semibold text-c-black/85 text-[13px] leading-7">
          {props.title}
        </span>
      </div>
